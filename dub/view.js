...
export function renderSheet(S){
  ...
  if (S.flow) {
    ...
    el.innerHTML = items.map(it => it.pin != null
      ? `<div class="pinflow" data-pin="${it.pin}">${pinFlowHTML(S, it.pin)}<button class="unpin" data-unpin="${it.pin}" title="このピンを外す（確認あり）" aria-label="ピンを外す">×</button></div>`
  ...
  items.forEach((it, i) => {
    const r = i + 2;
    if (it.pin != null) {
      h += `<div class="pintc" data-pin="${it.pin}" style="grid-row:${r};grid-column:1"><button class="unpin" data-unpin="${it.pin}" title="このピンを外す（確認あり）" aria-label="ピンを外す">×</button>${tc(it.pin)}</div>`;
      vis.forEach((li, k) => {
